...
export default class CreditCard extends Component {
    constructor(props){
        ...
        this.state = savedState && savedState.TabThree ?  savedState.TabThree : {};
        ...
        <div>
            ...
            <Style stylesheet={CREDITCARD_STYLE} namespace={this.stylePrefix} className={this.state && this.state.flip ? "cc-show-back" : "cc-show-front" }>
                <div className="cc-card cc-front">
                    ...
                    <MaskedInput mask="1111 1111 1111 1111" name="card" className="cc-text cc-number" size="20" value={this.state.card} required onChange={this.onChange}/>
                    <span className="cc-month-year-label cc-label">Month/Year</span>
                    <MaskedInput mask="11/11" name="month-year" className="cc-text2 cc-month-year" value={this.state['month-year']}  size="20" required onChange={this.onChange}/>
                </div>
                <div className="cc-card cc-back">
                    ...
                    <MaskedInput mask="111" name="cvv" className="cc-cvv" size="20" value={this.state.cvv} required onChange={this.onChange}/>
                </div>
    ...
    componentWillUnmount(){
        const shareableState = Object.assign(
            {},
            {
                card: this.state && this.state.card ? this.state.card.replace(/[^0-9]/g, '') : '',
                'month-year': this.state && this.state['month-year'] ? this.state['month-year'].replace(/[^0-9]/g, '') : '',
                cvv: this.state && this.state.cvv ? this.state.cvv : '',
            },
            {type: 'SET_CARD'}
        );

        store.dispatch(shareableState);
    }
